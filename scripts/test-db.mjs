...
const modes = ['--all', '--baseline', '--pilot-views-review', '--pilot-views-baseline', '--parent-upgrade-review', '--feedback-publication-review', '--player-journey-review'];
...
try {
  ...
  // Both depend on publish_player_feedback, so they are registered behind their
  // own flags rather than added to the default list — until this migration is
  // on main, --all cannot run them.
  const t2Suites = {
    '--feedback-publication-review': 'feedback_publication.sql',
    '--player-journey-review': 'player_journey.sql',
  };
  const suites = t2Suites[mode] ? [t2Suites[mode]]
